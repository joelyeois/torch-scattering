"""High-level Rytov-approximation wrapper.
...
def rytov(
    ...
    n_slices : int | None
        Number of terms to sum in the exponentiated Born series. If `None`
        (default), every slice of `potential` is treated individually
        (equivalent to `n_slices=Z`). If smaller than `Z`, slices are
        grouped into `n_slices` contiguous chunks via `chunk_slices` (all
        but the last chunk have the same size; the last absorbs the
        remainder), and each chunk's summed potential is propagated as one
        thicker slab. Must satisfy ``0 < n_slices <= Z``.
    ...
    plane and summed inside a single exponential, rather than being
    propagated incrementally slice-by-slice as in `multislice`:
    ``psi = exp(sum_c IFFT(FFT(i * sigma * dz * V_c) * H_c))``, where `V_c`
    ...
    the exponentiated Born series, reducing to the first Born approximation
    for small `V`. Faster than `multislice` (one FFT pass instead of one
    per chunk) but less accurate for thick specimens. `n_slices=None` is
    the most accurate setting; grouping trades accuracy for speed.
    ...
    return torch.exp(scattered.sum(dim=-3))
